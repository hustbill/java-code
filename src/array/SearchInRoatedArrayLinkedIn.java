...
public class SearchInRoatedArrayLinkedIn {
	...
	// items.
	// date : 2016.09.05 leetcode 81. search in roated sorted array II
	// 包含duplicates
	...
	/*
	 * 81. Search in Rotated Sorted Array II Follow up for
	 * "Search in Rotated Sorted Array": What if duplicates are allowed?
	 * 
	...
	public static boolean searchInRotatedArrayWithDuplicates(int[] nums, int target) {
		...
		while (start <= end) {
			...
			} else if (nums[start] > nums[mid]) { // right part is rotated
	...
	public static int findInRotatedArrayWithDuplicates(int[] nums, int target) {
		int start = 0, end = nums.length - 1;
