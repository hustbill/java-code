...
/**
 * Token bucket that listens on given port for incoming packets. After applying
 * shaping policy to packets they are sent to given address and port.
 */
public class TokenBucket implements Runnable {
	...
	/**
	 * Constructor. Creates a buffer, receiver, sender, and token generator
	 * class instances.
	 * 
	 * @param inPort
	 *            Port on which to listen for packets.
	 * @param outAddress
	 *            IP address to which packets are sent.
	 * @param outPort
	 *            Port to which packets are sent.
	 * @param maxPacketSize
	 *            Maximum size of packet that will be received.
	 * @param bufferCapacity
	 *            Capacity of buffer in bytes.
	 * @param bucketSize
	 *            Number of tokens that token bucket can hold.
	 * @param bucketRate
	 *            Token generating rate in tokens/sec.
	 * @param fileName
	 *            Name of file token bucket uses to record packet arrivals.
	 */
	public TokenBucket(int inPort, String outAddress, int outPort, int maxPacketSize, long bufferCapacity,
